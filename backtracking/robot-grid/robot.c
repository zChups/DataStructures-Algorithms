...
void FindPathsRec(const struct Grid* gcurr, size_t r, size_t c, int nstep, int* nsol) {
	...
	if (r < gcurr->rows - 1 && gcurr->cells[next_index] != -1) {
		...
		FindPathsRec(gcurr, r + 1, c, nstep + 1, nsol);
	...
	if (c < gcurr->cols - 1 && gcurr->cells[next_index] != -1) {
		gcurr->cells[next_index] = nstep;
		FindPathsRec(gcurr, r, c + 1, nstep + 1, nsol);
		gcurr->cells[next_index] = 0;
	}
